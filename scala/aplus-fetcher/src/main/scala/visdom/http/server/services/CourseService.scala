...
with APlusFetcherResponseHandler
{
    ...
    @Operation(
        ...
        parameters = Array(
            new Parameter(
                name = APlusServerConstants.ParameterCourseId,
        ...
        responses = Array(
            new ApiResponse(
                ...
                description = APlusFetcherDescriptions.CourseDataStatusAcceptedDescription,
                content = Array(
                    new Content(
            ...
            new ApiResponse(
                ...
                description = APlusFetcherDescriptions.CourseStatusInvalidDescription,
                content = Array(
                    new Content(
                        schema = new Schema(implementation = classOf[ResponseProblem]),
                        examples = Array(
                            new ExampleObject(
                                name = APlusFetcherDescriptions.CourseResponseExampleInvalidName,
                                value = APlusFetcherExamples.CourseResponseExampleInvalid
                            )
                        )
                    )
                )
    ...
    def getCourseDataRoute: RequestContext => Future[RouteResult] = (
        ...
        parameters(
            APlusServerConstants.ParameterCourseId.optional
        )
    ) {
        (courseId) => get {
