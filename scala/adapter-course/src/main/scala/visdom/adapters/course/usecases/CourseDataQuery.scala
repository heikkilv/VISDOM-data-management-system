                ...
                module => (
                    ...
                    module._links match {
                        case Some(links: ModuleLinksSchema) => links.exercises match {
                            case Some(exerciseIds: Seq[Int]) => queryOptions.exerciseId match {
                                case Some(exerciseId: Int) => exerciseIds.filter(id => id == exerciseId)
                                case None => exerciseIds
                            }
                            case None => Seq.empty
                        }
                        case None => Seq.empty
