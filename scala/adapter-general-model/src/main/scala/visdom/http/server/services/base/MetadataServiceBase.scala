...
@Path(ServerConstants.MetadataRootPath)
abstract class MetadataServiceBase(actorRef: ActorRef)(implicit executionContext: ExecutionContext)
extends Directives
with AdapterService
with MultiInputOptionsTrait
{
    ...
    @Operation(
        summary = GeneralAdapterDescriptions.MetadataEndpointSummary,
        description = GeneralAdapterDescriptions.MetadataEndpointDescription,
        parameters = Array(
            new Parameter(
    ...
    def getServiceRoute: RequestContext => Future[RouteResult] = (
        path(ServerConstants.MetadataPath) &
        parameters(
        ...
        (
            ...
            getRoute(
                actorRef,
                getMultiOptions(
                    ObjectTypes.TargetTypeMetadata,
                    page,
                    pageSize,
                    objectType,
                    query,
