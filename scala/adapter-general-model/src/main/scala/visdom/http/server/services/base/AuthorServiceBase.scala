...
@Path(ServerConstants.AuthorsRootPath)
abstract class AuthorServiceBase(actorRef: ActorRef)(implicit executionContext: ExecutionContext)
extends Directives
with AdapterService
with MultiInputOptionsTrait
{
    ...
    @Operation(
        summary = GeneralAdapterDescriptions.AuthorsEndpointSummary,
        description = GeneralAdapterDescriptions.AuthorsEndpointDescription,
        parameters = Array(
            new Parameter(
    ...
    def getServiceRoute: RequestContext => Future[RouteResult] = (
        path(ServerConstants.AuthorsPath) &
        parameters(
