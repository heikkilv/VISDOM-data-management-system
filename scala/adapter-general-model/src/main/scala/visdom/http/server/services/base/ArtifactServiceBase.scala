...
import visdom.http.server.options.MultiOptionsTrait
...
abstract class ArtifactServiceBase(actorRef: ActorRef)(implicit executionContext: ExecutionContext)
extends Directives
with AdapterService
    ...
    def getServiceRoute: RequestContext => Future[RouteResult] = (
        ...
        (
            ...
            getRoute(
                actorRef,
                getMultiOptions(
                    ObjectTypes.TargetTypeArtifact,
                    page,
                    pageSize,
                    objectType,
                    query,
